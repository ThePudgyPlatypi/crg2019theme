...
<div class="half-header-container grid-container full collaborative-response-graphic" style="background-image: url(<?php 
        ...
        <?php } else if (is_page('philanthropy')) { ?>
            <!-- video -->
                <video autoplay muted loop preload="metadata" class="half-banner-video philanthropy" <?php echo 'poster="'. get_the_post_thumbnail_url() .'"'?>>
                    <source src="<?php bloginfo('template_url'); ?>\dist\assets\images\header-Philanthropy-v3-small.mp4" type="video/mp4">
                </video> 
        <?php } ?>
